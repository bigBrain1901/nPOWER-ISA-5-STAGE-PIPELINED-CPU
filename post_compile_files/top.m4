...
\TLV
   ...
   //    o data memory
   |cpu
      \source top.tlv 41   // Instantiated from top.tlv, 205 as: m4+imem(@1)    // Args: (read stage)
         @1
            \SV_plus
      ...
      \source top.tlv 61   // Instantiated from top.tlv, 206 as: m4+rf(@2, @3)  // Args: (read stage, write stage) - if equal, no register bypass is required
         // Reg File
         @3
            /xreg[63:0]
         ...
         @2
            ?$rf_rd_en1
               $rf_rd_data1[63:0] = /xreg[$rf_rd_index1]>>2$value;
            ?$rf_rd_en2
               $rf_rd_data2[63:0] = /xreg[$rf_rd_index2]>>2$value;
            `BOGUS_USE($rf_rd_data1 $rf_rd_data2)
      \end_source  // Args: (read stage, write stage) - if equal, no register bypass is required
      \source /raw.githubusercontent.com/stevehoover/RISCVMYTHWorkshop/c1719d5b338896577b79ee76c2f443ca2a76e14f/tlvlib/riscvshelllib.tlv 50   // Instantiated from top.tlv, 207 as: m4+dmem(@4)    // Args: (read/write stage)
         // Data Memory
         @4
            /dmem[15:0]
